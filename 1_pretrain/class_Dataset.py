...
class llavadataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_processed = self.processor(images=img, return_tensors="pt")[
            "pixel_values"
        ]
        image_processed = image_processed.squeeze(0)

        a = self.tokenizer(
            koala_data[1]["gpt"], return_tensors="pt", return_attention_mask=False
        )
...
def collate_fn(batch):
    ...
    captions_padded = torch.nn.utils.rnn.pad_sequence(
        captions, batch_first=True, padding_value=50256
    )
